...
dff = rbind(cosmx_cancer,merscope_hbreast,
            cosmx_normal, xenium_hbreast)
...
for (cl in c("c1", "c5","c8")){
    ...
    p<- upset(df_mt,intersect=colnames(df_mt),
                 ...
                 width_ratio=0.2, height_ratio=1/2)+
        ggtitle(paste(anno_name,"cells"))+
        theme(plot.title = element_text(size = 20, face = "bold"))
    plt_lst[[cl]] <-p
}  
combined_plot <- wrap_plots(plt_lst, ncol = 3)
pdf(file.path(figure_technical_performance, "xenium_mg_ntiles.pdf"), height=8, width=22)
print(combined_plot)
dev.off()
