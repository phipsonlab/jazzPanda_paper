...
wk_path = here("scripts/main/cosmx_hlc_simulation_10k_spearman_result/")
file_nm = "spearman"
...
colSums(p_value_df[, 1:13] <0.05)/10000
colSums(adjp_df[, 1:13] <0.05)/10000

table(colSums(adjp_df[, 1:13] <0.05))
...
jpeg(file.path(comp_PA, paste0("adjpval_vs_correlation_scatter_10k_", file_nm,".jpg")), 
...
ggplot(merged_df, aes(x = statistic, y = log_padj,color=label)) +
  ...
  geom_vline(xintercept = 0.05, color = "grey", linetype = "dashed") +
    geom_label(data = quad_counts,
               ...
               color = "black"    ) +
  labs(x = "Spearman correlation", y = "-log10(adjusted p-value)"
       ) +
...
rawp_long <- p_value_df[, 1:13] %>%
  rownames_to_column("gene") %>%
  pivot_longer(-gene, names_to = "cluster", values_to = "raw_p")
...
merged_df <- left_join(rawp_long, obs_long, by = c("gene", "cluster"))

merged_df <- merged_df %>%
  mutate(log_praw = -log10(raw_p),
         flag_low_effect = raw_p < 0.05 & statistic > 0.05)  
merged_df$label = "Not significant"
merged_df[merged_df$flag_low_effect==TRUE,"label"] ="Significant"
merged_df$label = factor(merged_df$label, 
                         levels=c("Significant", "Not significant"))
...
merged_df <- merged_df %>%
  mutate(
    quadrant = case_when(
      statistic <= x_thresh & log_praw >= y_thresh ~ "Q1",  # Top-left
      statistic >  x_thresh & log_praw >= y_thresh ~ "Q2",  # Top-right
      TRUE ~ "Other"
    )
  )
merged_df$anno_name = factor(merged_df$anno_name, levels =cluster_names)
...
quad_counts <- quad_counts %>%
  left_join(label_positions, by = "quadrant")
jpeg(file.path(comp_PA, paste0("raw_pval_vs_correlation_scatter_10k_", file_nm,".jpg")),
    width=2500, height=2000,res=200)
ggplot(merged_df, aes(x = statistic, y = log_praw,color=label)) +
  geom_point(alpha = 0.6, size=0.01) +
    facet_wrap(~anno_name, ncol=4)+
    scale_color_manual(values=c("steelblue","black"))+
    scale_y_continuous(breaks = c(0, 1,-log10(0.05), 2,3), 
  ...
  geom_vline(xintercept = 0.05, color = "grey", linetype = "dashed") +
    geom_label(data = quad_counts,
               ...
               label.size = 0.3, label.r = unit(0.1, "lines"), fill = "white",   
               color = "black"    ) +
  labs(x = "Spearman correlation", y = "-log10(raw p-value)"
       ) +
        guides(colour = guide_legend(override.aes = list(size=4,alpha=1)))+
  theme_minimal(base_size = 13) +
  theme(panel.border = element_rect(color = "black", 
