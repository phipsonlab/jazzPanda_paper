...
# Marker gene comparision vs number of tiles (glm)
```{r}
...
cosmx_files <- list.files(path = result_path, 
                          pattern = "^cosmx.*\\.csv$", full.names = TRUE)
# Read all files into a list of data frames
...
## cosmx human liver cancer - correlation approach
```{r}

n_rows <- nrow(cosmx_merged_df)
n_cols <- 13
df_names <- paste0("gr", seq(10, 100, by = 10),"_df")

for (nm in df_names) {
  df <- as.data.frame(matrix(FALSE, nrow = n_rows, ncol = n_cols))
  rownames(df) <- cosmx_merged_df$gene
  colnames(df) <- paste("c", 1:13, sep="")
  assign(nm, df)
}


for (bin_l in seq(10, 100, 10)){
    file_nm = paste0("cosmx_hliver_cancer_perm_lst_gr",bin_l,".Rds")
    perm_lst = readRDS(file.path(result_path, file_nm))
    obs_corr = get_cor(perm_lst)
    perm_res = get_perm_adjp(perm_lst)
    df <- get(paste0("gr", bin_l,"_df"))
    for (cl in paste("c", 1:13, sep="")) {
        perm_cl=intersect(row.names(perm_res[perm_res[,cl]<0.05,]),
              row.names(obs_corr[obs_corr[, cl]>0.05,]))

        df[perm_cl,cl] = TRUE
    }
    assign(paste0("gr", bin_l,"_df"), df) 
...
plt_lst = list()
for (cl in setdiff(clusters_df$cluster, c("c9", "c11", "c12","c13"))){
    ...
    for (bin_l in seq(10, 100, by = 10)) {
        df_name <- paste0("gr", bin_l, "_df")  
        gr_df =get(df_name)                
        gr_rows <- rownames(gr_df[gr_df[, cl] == TRUE, , drop = FALSE])
        col_nm <- paste0(bin_l, "x", bin_l)
        df_mt[gr_rows, col_nm] <- TRUE
    }
...
pdf(file.path(comp_PA, "cosmx_mg_ntiles_corr.pdf"),height=30, width=15)
combined_plot 
dev.off()

```
## cosmx human liver cancer - linear modelling approach
```{r}
...
for (cl in clusters_df$cluster){
    ...
    for (bin_l in seq(10, 100, by = 10)) {
        ...
        gr_genes = cosmx_merged_df$gene[which(cosmx_merged_df[[cl_col]] == cl)]  # which() = NA-safe
        col_nm   = paste0(bin_l, "x", bin_l)
    ...
    rs= rowSums(df_mt)
    core_n = sum(rs == ncol(df_mt))      # marker in ALL 10 bins
    union_n = sum(rs >= 1)            # marker in >=1 bin
    
    pj = combn(ncol(df_mt), 2, function(ij) {
        a = df_mt[, ij[1]]; b = df_mt[, ij[2]]; u = sum(a | b)
        if (u > 0) sum(a & b) / u else NA_real_
    })
    
    rec_bins = colnames(df_mt)[seq(10,100,by=10) >= 40]   # 20x20 ... 100x100
    rec_res  = df_mt[, rec_bins, drop=FALSE]
    pj_big = combn(ncol(rec_res), 2, function(ij){
        a=rec_res[,ij[1]]; b=rec_res[,ij[2]]; u=sum(a|b); if(u>0) sum(a&b)/u else NA_real_
    })


    jac_summary = rbind(jac_summary, data.frame(
        cluster=cl,
        anno=anno_name,
        shared=core_n,
        union= union_n,
        agreement = round(mean((rs[rs >= 1] - 1) / (ncol(df_mt) - 1)), 3),
        multiset_jaccard = if (union_n > 0) round(core_n / union_n, 3) else NA_real_,
        mean_pairwise = round(mean(pj, na.rm = TRUE), 3),
        mean_pairwise_big = round(mean(pj_big, na.rm=TRUE), 3)
    ))
...
for (cl in setdiff(clusters_df$cluster, c("c11","c9","c12"))){
    ...
    for (bin_l in seq(10, 100, by = 10)) {
        cl_col=paste0("top_cluster_gr", bin_l)
        gr_genes=cosmx_merged_df[cosmx_merged_df[, cl_col] == cl, "gene"]
        col_nm=paste0(bin_l, "x", bin_l)
        df_mt[gr_genes, col_nm]=TRUE
    }
    anno_name = clusters_df[clusters_df$cluster==cl,"anno"]
...
combined_plot=wrap_plots(plt_lst, ncol = 2)
pdf(file.path(comp_PA, "cosmx_mg_ntiles_glm.pdf"),height=30, width=15)
combined_plot 
dev.off()

```

## Xenium huamn breast cancer samples - Linerar modelling approach
```{r}
...
jac_summary = data.frame()      # <-- collector, one row per cluster
plt_lst = list()
...
for (cl in clusters_df$cluster){
    ...
    for (bin_l in seq(10, 100, by = 10)) {
        cl_col   = paste0("top_cluster_gr", bin_l)
        gr_genes = xenium_merged_df$gene[which(xenium_merged_df[[cl_col]] == cl)]  # which() = NA-safe
        col_nm   = paste0(bin_l, "x", bin_l)
        df_mt[gr_genes, col_nm] = TRUE
    }
    ...
    rec_bins = colnames(df_mt)[seq(10,100,by=10) >= 20]   # 20x20 ... 100x100
    rec_res  = df_mt[, rec_bins, drop=FALSE]
    pj_big = combn(ncol(rec_res), 2, function(ij){
        a=rec_res[,ij[1]]; b=rec_res[,ij[2]]; u=sum(a|b); if(u>0) sum(a&b)/u else NA_real_
    })
...
combined_plot <- wrap_plots(plt_lst, ncol = 2)
pdf(file.path(comp_PA, "xenium_mg_ntiles.pdf"), height=30, width=15)
combined_plot 
dev.off()
```
