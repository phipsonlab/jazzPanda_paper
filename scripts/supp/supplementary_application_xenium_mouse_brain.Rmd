...
# 
# jpeg(paste0("xy_persample.jpg"), width=1000*5, height=700*3, res=200)
# 
...
for (cl in cluster_names){
    ...
    p <-ggplot(data_lst, aes(x = rk, y = y, color = type)) +
        ...
        labs(title = ct_nm, x = "Rank of marker genes",
             y = "Cumulative count of genes with correlation >0.6",
             color = NULL) +
...
pdf(file.path(comp_PA, paste0(data_nm, "_mg_compare_upset.pdf")),
     height =4* (ceiling(length(cluster_names)/3)), width = 12)
combined_plot 
dev.off()
```
